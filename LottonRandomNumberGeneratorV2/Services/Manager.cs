...
namespace LottonRandomNumberGeneratorV2.Services
{
    public class Manager
    {
        readonly List<IAlgorithm> _algorithms;

        public Manager(List<IAlgorithm> algorithms)
        {
            this._algorithms = algorithms;
        }

        public List<string> ManageNumbers(Game game)
        {
        ...
        public string FormatNumbersIntoString(List<List<int>> mainNumbers, List<List<int>> bonusNumbers)
        {
            ...
            for (int i = 0; i < mainNumbers.Count(); i++)
            {
                ...
                if (bonusNumbers != null && bonusNumbers[i].Any())
                {
                    sb.Append(" | ");
